...
function EnhancedZoneBookingPage() {
    ...
    try {
      ...
      try {
        ...
        if (riskResponse.data.loaded && riskResponse.data.locations) {
          ...
          // Only show storage zones (totalSlots > 0)
          const storageZones = enrichedZones.filter(zone => zone.totalSlots > 0);
          setZones(storageZones);
        } else {
          // Only show storage zones (totalSlots > 0)
          const storageZones = zonesData.filter(zone => zone.totalSlots > 0);
          setZones(storageZones);
        }
      } catch (riskError) {
  ...
  const loadRiskForBooking = async () => {
    try {
      ...
      // 404 can happen if the ML service doesn't expose a per-zone risk endpoint.
      // Keep the UI clean: no red console errors, just hide the risk panel.
      const status = error?.response?.status;
      if (status && status !== 404) {
        console.warn('ML risk endpoint unavailable:', status);
      }
      setRiskData(null);
    }
  };
  ...
  const addItem = () => {
    setItemsToStore([...itemsToStore, { itemName: '', description: '', category: 'Other' }]);
  };

  const removeItem = (index) => {
    setItemsToStore(itemsToStore.filter((_, i) => i !== index));
  };

  const updateItem = (index, field, value) => {
    const updated = [...itemsToStore];
    updated[index][field] = value;
    setItemsToStore(updated);
  };
  ...
  const createBooking = async () => {
    ...
    // Validate items
    const validItems = itemsToStore.filter(item => item.itemName.trim() !== '');

    if (validItems.length === 0) {
      alert('Please add at least one item to store');
      return;
    }

    if (slotsToBook < validItems.length) {
      alert(`You need at least ${validItems.length} slots for ${validItems.length} items`);
      return;
    }
    ...
    try {
      ...
      setBookingDate('');
      setSlotsToBook(1);
      setItemsToStore([{ itemName: '', description: '', category: 'Other' }]);
      loadZones();
      loadMyBookings();
    } catch (error) {
  ...
  const storeItems = async (bookingId) => {
    try {
      await api.post(`/zone-bookings/${bookingId}/store-items`);
      alert('Items stored successfully!');
      loadMyBookings();
    } catch (error) {
      alert('Error storing items: ' + (error.response?.data?.error || error.message));
    }
  };
  ...
  return (
    <div style={{ padding: '20px', fontFamily: 'Arial, sans-serif' }}>
      <h1 className="d-flex align-items-center gap-2">
        ...
        Zone Slot Booking for Item Storage
      </h1>
      <p style={{ color: '#6b7280', marginBottom: '20px' }}>
        Book storage slots with ML-powered risk assessment
      </p>

      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px', marginTop: '20px' }}>
        {/* Available Zones */}
        <div>
          <h2>Available Storage Zones</h2>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start' }}>
                  <div>
                    ...
                    <p style={{ margin: '5px 0', fontSize: '12px', color: '#6b7280' }}>
                      Risk Score: {(zone.riskScore * 100).toFixed(0)}%
                    </p>
                  </div>
                  <div
                    style={{
              ...
              >
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start' }}>
                  <div style={{ flex: 1 }}>
                    ...
                    {booking.itemsToStore && booking.itemsToStore.length > 0 && (
                      <div style={{ marginTop: '10px' }}>
                        <strong>Items:</strong>
                        <ul style={{ margin: '5px 0', paddingLeft: '20px' }}>
                          {booking.itemsToStore.map((item, idx) => (
                            <li key={idx} style={{ fontSize: '14px', color: '#6b7280' }}>
                  ...
                  <div style={{ textAlign: 'right' }}>
                    ...
                    {booking.status === 'APPROVED' && (
                      <div style={{ display: 'flex', gap: '5px', flexDirection: 'column' }}>
                        <button
                          onClick={() => storeItems(booking._id)}
                          style={{
                            padding: '5px 10px',
                            backgroundColor: '#3b82f6',
                            color: 'white',
...
export default EnhancedZoneBookingPage;
